...
public class GITChangeExtractionThread extends Thread {
  ...
  public GITChangeExtractionThread(final CPAConfig config, final Revision revision,
      final Repository repository, final PlotWalk revWalk, final ObjectReader reader) {
    this.config = config;
  ...
  public void run() {
    ...
    synchronized (LOCK) {
      try {
        final ObjectId commitId = this.repository.resolve(this.revision.id);
    ...
    try {
      ...
      final LCS lcs = new LCS(config, repoPath, this.revision);
      ...
      for (final DiffEntry entry : diffEntries) {
        ...
        if (!languages.stream()
            ...
            || !languages.stream()
                .anyMatch(lang -> lang.isTarget(newPath))) {
          continue;
        }
  ...
  private String readText(final AbbreviatedObjectId blobId) {
    try {
      final ObjectLoader loader = this.reader.open(blobId.toObjectId(), Constants.OBJ_BLOB);
