...
public class CPAConfig {
  ...
  static {
    ...
    {
      final Option source = new Option("cachedb", "cachedb", true, " parsetree cache file generated by nitron");
      source.setArgName("file");
      source.setArgs(1);
      source.setRequired(false);
      OPTIONS.addOption(source);
  ...
  public File getCacheDB() {
    String optName = "cachedb";
    if (!this.commandLine.hasOption(optName)) return null;

    final File file = new File(this.commandLine.getOptionValue(optName));
    if (!file.exists()) {
      System.err.println("specified path by option \"" + optName + "\" does not exist.");
      System.exit(0);
    }
    return file;
  }
